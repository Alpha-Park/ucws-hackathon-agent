...
function runShoppingWorkflow(userId, sessionId, message) {
  ...
  if (!products.length) {
    products = rankProducts(message, { maxPrice: budget, limit: 5 });
    category = null;
    toolCalls.push({
      tool: "fallback_rank_products",
      input: { query: message, max_price: budget, limit: 5 },
      status: "completed",
      summary: `${products.length} fallback candidates ranked after relaxing category`,
    });
  }
  ...
  return {
    ...
    products,
    collection: listCollection(userId),
